...
`errs` is a Tier 0 kernel package (see spec 0002 §6). It sits at the bottom of the import graph: it imports nothing from `github.com/nathanbrophy/glacier/...`, only the six stdlib packages it uses directly (`errors`, `fmt`, `iter`, `runtime`, `strconv`, `strings`).
