...
// When set, Run is a no-op — the consumer drives the shared Animator themselves.
// The SpinnerAnimator is registered into the shared Animator at construction time.
func WithSpinnerAnimator(a *Animator) SpinnerOption

// SpinnerAnimator is a self-contained spinner animation. It satisfies Animation
// (usable with Animator.Add) and also exposes Run and Close for standalone use.
// Concurrency: all exported methods are goroutine-safe.
type SpinnerAnimator struct{ /* unexported fields */ }
...
// Run starts the spinner self-contained. See §Self-contained animation primitives.
func (s *SpinnerAnimator) Run(ctx context.Context) error

// Close stops the spinner. Idempotent; returns nil always.
...
//
// *SpinnerAnimator satisfies Animation and can be passed to Animator.Add.
...
// When set, Run is a no-op — the consumer drives the shared Animator themselves.
// The Progress is registered into the shared Animator at construction time.
...
// Run starts the progress animation self-contained. See §Self-contained animation primitives.
func (p *Progress) Run(ctx context.Context) error

// Close stops the Progress animation. Idempotent; returns nil always.
// Equivalent to Done() plus stopping any owned internal Animator.
...
// When set, Run is a no-op — the consumer drives the shared Animator themselves.
// The StatusBar is registered into the shared Animator at construction time.
...
// Run starts the StatusBar animation self-contained. See §Self-contained animation primitives.
...
// StatusBar is closed by the caller via its Handle or Close.
func (s *StatusBar) Render() ([]string, bool)

// Close stops the StatusBar's render participation and releases resources.
// If an internal Animator was created by Run, it is stopped.
...
// Because it embeds *Progress, all Progress methods are promoted: Run, Close,
// Done, Set, Increment, and Render.
...
// (indeterminate mode). To inject a shared Animator, pass WithProgressAnimator(a).
...
// When Source returns io.EOF, Done is called automatically.
// Goroutine-safe.
func (d *DownloadProgress) Read(p []byte) (int, error)
```

### Self-contained animation primitives

Each of `Spinner`, `Progress`, `StatusBar`, and `DownloadProgress` is a **self-contained animator** in addition to being an `Animation` value that can be registered with a shared `Animator`. Consumers can drive a single animation without constructing an `Animator` themselves.

**Pattern A — standalone (self-contained):**

```go
sp := term.Spinner("loading users")
go sp.Run(ctx)   // launches an internal Animator; blocks until ctx cancels or Close
defer sp.Close() // idempotent; stops the internal Animator
```

**Pattern B — shared Animator (multi-animation coordination):**

```go
a := term.NewAnimator(slog.Default())
sp := term.Spinner("phase 1", term.WithSpinnerAnimator(a)) // registered immediately
p  := term.NewProgress(total, term.WithProgressAnimator(a))

go a.Run(ctx)     // drives both animations with one log-interception point
defer a.Close()
```

When `WithSpinnerAnimator` / `WithProgressAnimator` / `WithStatusBarAnimator` is provided:
- The primitive is registered into the shared Animator at construction time.
- `Run` is a **no-op** (returns nil immediately).
- The consumer calls `sp.Close()` / `p.Close()` to mark the animation done so the Animator removes it.

**Internal Animator construction:** when `Run` creates an internal Animator, it uses `slog.Default()` when that logger's handler is not already intercepted by another running Animator. If it is already intercepted (concurrent animators all sharing the global default), a fresh logger backed by `os.Stderr` is used instead. This prevents the `"handler already intercepted"` error under concurrent self-animating primitives.

**`Close()` contract:**
- Always idempotent (returns nil on repeated calls).
- If the primitive owns an internal Animator, `Close()` stops it.
- If using an injected Animator, `Close()` marks the animation done (sets `done=true`) so its next `Render()` returns `done=true`.
- `Progress.Close()` is equivalent to `Done()` plus stopping any owned Animator. Both `Done()` and `Close()` are valid completion paths for Progress.
...
### Self-contained spinner (standalone use)
...
func ExampleSpinnerAnimator_selfContained() {
    ctx, cancel := context.WithCancel(context.Background())

    // No Animator setup required — the spinner creates an internal one.
    sp := term.Spinner("loading users")
    go sp.Run(ctx) // blocks internally; returns when ctx cancels or Close is called
    defer sp.Close()
...
### D8 — Self-contained animation primitives (project memory `project_term_self_animating_primitives`)

Each animation primitive gains `Run(ctx) error` and `Close() error` so consumers can use a single animation without boilerplate `Animator` setup. The shared-Animator path is preserved via `WithSpinnerAnimator` / `WithProgressAnimator` / `WithStatusBarAnimator` options.

Key design decisions:

- **`Spinner` return type changed from `Animation` to `*SpinnerAnimator`.** `*SpinnerAnimator` satisfies `Animation`, so existing `animator.Add(term.Spinner("x"))` callers are unaffected. The concrete return type is required so callers can access `Run` and `Close`.
- **`newInternalLogger()` helper.** When `Run` creates an internal Animator, it checks whether `slog.Default()`'s handler is already an `interceptHandler` (i.e., another Animator is running). If so, it falls back to a fresh `slog.NewTextHandler(os.Stderr, nil)` rather than failing with `"handler already intercepted"`. This makes concurrent self-animating primitives safe.
- **`DownloadProgress` inherits via embedding.** `DownloadProgress` embeds `*Progress`, so `Run`, `Close`, `Done`, and the animator injection are all promoted without duplicating logic. Consumers pass `WithProgressAnimator(a)` as a `ProgressOption`.
- **`Progress.Close()` added.** `Progress` previously had only `Done()`. `Close()` is added as an alias that also stops any owned internal Animator. Both paths are valid.
...
- **§23.16:** `term.Animator.Close() error` is idempotent, restores the logger handler, and uses `errs.Join` for accumulated sub-errors. `term.StatusBar.Close() error` added per the audit. `term.Progress.Close() error` added as part of the self-animating primitives refactor (D8); it is equivalent to `Done()` plus stopping any owned internal Animator. `SpinnerAnimator.Close() error` added likewise.
