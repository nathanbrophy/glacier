...
implementing-commits: [de3090f, 7bde59e]
...
- Remain Tier 0 (kernel): import only stdlib, `option`, `errs`, and `term`.
...
flowchart TB
    ...
    subgraph T0ext[Tier 0 :  extended]
        term
    end
...
**Allowed imports (NF8):** stdlib (`context`, `io`, `log/slog`, `os`, `strings`, `sync`, `time`), `github.com/nathanbrophy/glacier/option`, `github.com/nathanbrophy/glacier/errs` (optional at v0; the tier-0 allowance from spec 0002 D13 permits it for future use without a spec amendment), `github.com/nathanbrophy/glacier/term`.
...
│                       WithLevel; WithSource; WithColor; glacierHandler type
...
**Handler internals.** `glacierHandler` wraps a stdlib `slog.Handler` and overrides `Handle(ctx, record)` to:
...
**Lifecycle.** `log` has no stateful types requiring `Close`. `glacierHandler` is created once and holds no I/O resources of its own :  the `io.Writer` it wraps is the caller's responsibility.
...
func WithLogger(l *slog.Logger) option.Option[loaderConfig] {
	return option.OptionFunc[loaderConfig](func(c *loaderConfig) error {
		if l != nil {
			c.logger = l
		}
		return nil
...
| 19 | With attaches attrs to handler output | F5 | unit | `With(ctx, attr)` → attr in record from glacier handler. | `TestWithAttachesAttrs` |
...
| 45 | Concurrent logging :  no races | NF5 | race | 100 goroutines log via glacier handler; output well-formed; `-race` clean. | `TestHandlerConcurrent` |
...
- `github.com/nathanbrophy/glacier/term` (Tier 0; in-tree)
...
**`term` surface.** TTY detection (`isTerminal(fd)`) is a syscall-based check. The implementation must not cache the result across process lifetime in a way that allows TOCTOU :  color mode is resolved once at `NewHandler` call time and stored immutably in the handler struct.
...
Re-checking `os.Getenv("GLACIER_NO_COLOR")` on every log call would add a syscall to the hot path and would be surprising (env vars are not expected to change during program execution). The TTY check via `term` is also a syscall. Both are resolved once at `NewHandler` call time. Decision: resolve at construction.
...
8. **Import graph audit.** `go list -deps ./log/` contains no tier-1, tier-2, or non-internal glacier packages. The `internal/laytest/layering_test.go` invariant test (F1–F7 from spec 0002 §6) passes.
